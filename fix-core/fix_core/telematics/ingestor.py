"""
Telematics Ingestor — Phase 13C.

Converts raw machine sensor data into normalized EvidencePackets and
...
Architecture:
  ...
  4. Safety   — evaluate numeric thresholds → SafetyAlert objects
  5. Inspect  — helpers for admin/debug visibility

The caller (API layer) handles DB persistence and session linkage.
"""
...
class NormalizationResult:
    ...
    def to_session_dicts(self) -> tuple[list[dict], list[dict]]:
        """
        Serialize normalized signals and safety alerts to the dict format expected
        by DiagnosticSession.evidence_log and DiagnosticSession.safety_flags.

        Returns:
            (evidence_dicts, safety_dicts) — ready to extend session lists.
        """
        evidence_dicts = [p.to_dict() for p in self.normalized_signals]
        safety_dicts = [a.to_dict() for a in self.safety_alerts]
        return evidence_dicts, safety_dicts
...
def validate_payload(data: dict) -> ValidationResult:
    ...
    Args:
        data: Raw dict from the ingest request body.
    ...
    # Optional numeric fields — validate ranges if present
    ...
    # fault_codes must be a list of strings
...
def normalize_telemetry(payload: TelemetryPayload) -> NormalizationResult:
    ...
    Steps:
    1. Evaluate each sensor field against thresholds → named signals
    2. For each signal, create an EvidencePacket (source="sensor_future")
    3. Evaluate safety thresholds → SafetyAlert objects (no LLM)
    4. Return all in a NormalizationResult

    Args:
        payload: Validated TelemetryPayload.

    Returns:
        NormalizationResult with packets, alerts, and debug info.
    """
    ...
    if payload.fault_codes:
        ...
        code_str = ", ".join(payload.fault_codes[:10])   # cap display at 10
        packets.append(_make_packet(
...
def describe_normalization(result: NormalizationResult) -> dict:
    ...
    Includes:
    - raw sensor values
    - list of signals that fired with their normalized keys and certainty
    - safety alerts with level and message
    - evidence packet summary

    Args:
        result: NormalizationResult from normalize_telemetry().
